...
export interface BatchOpedUploadInfo {
  commitTitle: string
...
/**
 * 构造 GitHub 批量上传页面 URL，自动填充 commit 标题 (message) 与扩展描述 (description)
 */
export function buildBatchOpedGithubUploadUrl(
  subjects: Record<number, SubjectCustomOped>,
): BatchOpedUploadInfo {
  ...
  if (subjectCount === 0) {
    const defaultTitle = 'feat(data): batch upload OP/ED timestamps'
    return {
      commitTitle: defaultTitle,
      commitDescription: '',
      url: 'https://github.com/uerax/bangumi-oped/upload/data',
    }
  }
  ...
  for (const [idStr, sub] of entries) {
    const subId = Number(idStr)
    subIds.push(subId)
    const markedEps = Object.keys(sub.episodes)
      .map(Number)
      .filter((n) => Number.isFinite(n) && n > 0)
      .sort((a, b) => a - b)
    const epCount = markedEps.length
    totalEps += epCount
    const epRangeStr = epCount > 0 ? ` (ep ${formatRangeList(markedEps)})` : ''
    const nameStr = sub.subjectName ? `《${sub.subjectName}》` : `Subject ${subId}`
    subSummaries.push(`- ${subId} ${nameStr}: ${epCount} 集${epRangeStr}`)
  }

  // 1. Commit 标题 (简洁精炼: feat(data): upload OP/ED for [627136, 627132])
  const commitTitle =
    subIds.length > 0
      ? `feat(data): upload OP/ED for [${subIds.join(', ')}]`
      : 'feat(data): upload OP/ED timestamps'

  // 2. Commit 详细描述 (Extended description)
  const commitDescription = [
    `Batch upload OP/ED timestamp data generated by Animaku:`,
    '',
    ...subSummaries,
    '',
    `Total: ${subjectCount} subjects, ${totalEps} episodes marked.`,
  ].join('\n')

  const url = 'https://github.com/uerax/bangumi-oped/upload/data'
