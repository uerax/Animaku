...
- 描述：
  1. **保留原有视觉与弹窗样式**：
     - 严格保持移动端弹幕面板原有的居中弹窗定位与全套视觉样式（`fixed inset-0 m-auto w-[88%] max-w-[22rem] h-[65dvh] max-h-[26rem] ...`），不改动任何原有表单与滑块布局；
  2. **多源管理与状态条功能补齐**：
     - 在移动端居中面板头部补充状态栏与弹幕总数统计（`已加载 · 共 N 条 · 显示 M 条`）；
     - 在移动端居中面板底部补齐 `SourcesFooter` 弹幕源管理胶囊（DandanPlay / Bilibili / XML 导入），支持一键开关与条数展示，解决外部状态条移除后的多源切换断层问题；
     - 修正搜索 Tab 内提示文案为「可在面板底部开关各源」。
