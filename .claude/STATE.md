...
## [2026-08-18] 移动端弹幕面板现代流媒体抽屉重构与多源管理功能补齐
- 状态：已完成
- 优先级：P1
- 描述：
  1. **现代流媒体双模态抽屉体系（Bottom Sheet & Right Drawer）**：
     - **竖屏模式**：对标主流移动端流媒体（B 站 / YouTube），升级为贴合大拇指热区的底部抽屉（`inset-x-0 bottom-0 max-h-[82dvh] rounded-t-3xl`），带顶部药丸 Handle 与 `slide-in-from-bottom` 平滑升起动效；
     - **横屏全屏模式**：自适应切换为右侧滑入抽屉（`right-0 top-0 bottom-0 w-88 max-w-[85vw] rounded-l-2xl`），不遮挡中央主播放画面；
     - **背景交互**：半透明磨砂 Backdrop 遮罩，轻触背景即可平滑关闭。
  2. **多源管理与状态条完整性补齐**：
     - 在移动端面板头部补齐状态栏与弹幕总数统计（`已加载 · 共 N 条 · 显示 M 条`）；
     - 在面板底部补齐移动端触摸优化的 `SourcesFooter` 弹幕源管理胶囊（DandanPlay / Bilibili / XML 导入），支持一键开关与条数展示，彻底解决此前移除外部状态条后移动端无法切源的断层问题；
     - 修正搜索 Tab 内过时的「源开关在播放器下方」文案。
  3. **移动端触控交互与排版质感全面升维**：
     - **开关卡片**：「显示弹幕」与「弹幕精简」升级为触摸舒适的卡片式 Switch；
     - **滑块轨道**：优化透明度、字号、速度、区域 4 大滑块卡片，扩大 Touch Target，数值实时高亮显示；
     - **时间偏移微调**：补充 `[-0.5s]` 与 `[+0.5s]` 触控微调按钮，无需频繁调出软键盘；
     - **弹幕类型选择**：重构为 4 列等宽触控网格（滚动 / 顶部 / 底部 / 彩色）。
- 涉及文件：apps/web/src/player/DanmakuPanel.tsx, .claude/STATE.md
- 备注：`pnpm typecheck` 全仓 0 错误通过，`pnpm build:web` 全量打包编译通过。

## [2026-08-18] 移除播放页面多余的外部弹幕状态条
- 状态：已完成
- 优先级：P2
- 描述：
  - **移除播放器下方独立弹幕状态条**：从 `apps/web/src/pages/WatchPage.tsx` 中彻底移除播放器下方占位的 `弹幕 [状态] [源Chips]` 栏；
  - **功能完整性说明**：播放器内置的弹幕设置面板（`[弹+⚙️]`）已具备完备的弹幕源（DandanPlay / Bilibili / 本地导入）开关、数量展示与多源管理能力，移除外部冗余状态条后播放页面布局更纯粹紧凑、视觉重心更加聚焦于播放器与选集/简介区。
