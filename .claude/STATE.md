...
- 优先级：P2
- 描述：
  1. **纯前端瞬时类型过滤与分类计数徽标 (SearchPage.tsx)**：
     - 采用方案 A 纯前端即时过滤机制，复用上游原生的 `item.type` 字段（`type === 2` 为动漫，`type !== 2` 为三次元/非动漫），零新增网络请求，0ms 无缝响应；
     - 分类过滤胶囊默认选中「动漫」类型，同时提供 `[ 全部 (20) ]`、`[ 动漫 (17) ]`、`[ 非动漫 (3) ]` 三段式胶囊过滤按钮组；
     - 关键字切换时自动重置为默认「动漫」视图。
  2. **基于 airDate 的放送时间即时重排**：
     - 在过滤工具栏右侧新增排序控制胶囊：`[ 最新放送 ]`、`[ 默认匹配 ]`、`[ 最早放送 ]`；
     - 排序默认调整为「最新放送」（`date-desc`），纯前端基于每条数据的 `item.airDate` 进行精确日期比较重排，最新上映作品优先排在最前，无日期的条目自动沉底；
     - 0 额外网络开销，切换 0 延迟，切换搜索关键词时自动重置为默认「最新放送」。
  3. **智能空状态引导与自愈**：
     - 当搜索关键词在默认「动漫」分类无结果但在「非动漫」中有结果时（如搜真人剧/影视作品），明确提示并展示「查看非动漫 (X)」与「查看全部 (X)」快捷跳转按钮，杜绝误判无内容；
     - 总结果为 0 时统一展示标准无结果提示。
  4. **全链路验证**：
     - 全仓类型检查 `pnpm typecheck` 0 报错；
     - Shared 68 项单测、Server 108 项单测、Web 30 项单测全部通过；
     - 前端生产构建 `pnpm build:web` 成功；
     - 版本号保持在当次任务的 `v1.13.1`。
- 涉及文件：
  ...
  - packages/shared/package.json
  - packages/shared/src/version.ts
- 备注：严格限制改动范围在搜索页面，未扩散到底层 API 与通用组件。
...
- 优先级：P3
- 描述：
  1. **移除自定义规则与规则库在线安装过时宣称**：
     - 去除徽章中的 `AniBaka-anx--rule/2`，更换为通用的高质量直链徽章 `Direct_Stream-1080P`；
     - 移除“支持 AniBaka 与 Kazumi 双规则生态”、“支持 AniBaka (34+ 现代直链源) 与 Kazumi 传统规则 在线安装更新，支持多线路探活看板”等已下线的历史规则描述；
     - 核心特性重构为以当前架构为准的“内置精选多源与快捷换源”（内置优质多源直连秒开、鼠标/触控拖拽排序设主源、播放页无缝多源聚合与快捷切换抽屉）；
     - 特别致谢中精简对 AniBaka、Kazumi 的描述为单纯的设计与灵感参考，避免外部规则库集成的误解；
     - 同步更新系统架构文档 (`docs/architecture.md`) 中的章节标题与兼容描述。
  2. **消除潜在版权风险与第三方品牌强绑定**：
     - 将中英文 README 中所有涉及“B 站同款番剧推荐流”、“B 站风格推荐流”等名称统一修正为“智能番剧推荐流”（Smart Anime Recommendations），彻底规避第三方版权侵权风险与名称争议。
  3. **中英双语文档与架构设计同步核准 (`README.md`, `README.en.md`, `docs/architecture.md`)**：
     - 逐行比对并同步修订 `README.en.md` 对应章节；
     - 全文检索确认无任何残存的过时规则库与侵权命名。
  4. **版本规范执行**：
     - 本次修改纯属 Markdown 文档合规与核准优化，无实际业务代码改动，符合纯文档豁免准则，版本号保持 `v1.5.17` 不变。
- 涉及文件：
  - README.md
  - README.en.md
