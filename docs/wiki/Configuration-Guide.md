...
| :--- | :--- | :--- | :--- |
| **运行时变量 (Runtime)** | `PORT`, `PROXY_TOKEN`, `BANGUMI_API` 等 | 服务端进程启动时动态读取 | 修改 `.env` 后执行 `docker compose restart` 即可热生效 |
...
如果你是第一次使用 Docker 部署，大多数选项留空使用默认值即可。以下是仅有的 3 个最核心配置项：

```env
# 1. 宿主机访问端口（默认 8787，若被占用可修改，如 9000）
PORT=8787

# 2. 公网访问代理保护密码（公网部署强烈建议填写，内网 NAS 可留空）
PROXY_TOKEN=set_a_strong_password_here

# 3. 国内网络优化（默认 mirror 即可，无需翻墙畅享 Bangumi 数据）
BANGUMI_API=mirror
...
> ⚠️ **公网 VPS 部署警示**：视频播放绝大多数情况下直接走客户端直连源站 CDN，但当某些源需要反代请求头或 M3U8 切片重写时，会经由服务端代理通道。若不加密码保护，任何人都能调用你的 VPS 转发流量，可能导致流量被刷爆。
...
- **生效时机**：运行时
- **说明**：是否允许公网访客使用代理转发服务：
  - `1`（默认）：允许外部访客在满足安全鉴权（如配置了 `PROXY_TOKEN` 时需携带口令）的情况下调用规则检索与媒体重写；
  - `0`：严禁所有公网 IP 访问代理通道，仅限 `127.0.0.1` 本机及内网私有网段（192.168.x.x 等）访问。
...
- **生效时机**：运行时
- **说明**：管理员服务器代理授权密钥。
  - **留空时**：任何访问站点的用户都可使用代理通道（适合本地 NAS 或内网自建）；
  - **配置密码后**：服务端代理通道自动上锁！未授权访客打开网页后，若遇到需代理的源会提示未解锁；管理员在前端「设置」页面输入此 Token 即可一键解锁，同时服务端在处理请求时会验证该凭证。

### `ADMIN_SECRET`
- **默认值**：留空（未配置时默认回退到 `PROXY_TOKEN`）
- **类型**：字符串密码
- **生效时机**：运行时
- **说明**：高危管理员 API 鉴权密钥（如手动触发全站 IndexNow 差量提交、主动清除系统缓存等管理端点）。与普通访客代理口令物理隔离。

### `MEDIA_FULL_PROXY`
- **默认值**：`0` (关闭，仅转写 M3U8 列表，媒体切片直连 CDN)
- **类型**：布尔值 (`1` / `0`)
- **生效时机**：运行时
- **说明**：媒体全流量中转隧道总闸门：
  - `0`（推荐）：混合转发模式。服务端**仅代理重写文本类 M3U8 索引列表与防盗链密钥**，实际占用大带宽的 `.ts` 视频切片强制由客户端浏览器直连源站 CDN，**零消耗服务器中转流量**；
  - `1`：全量隧道模式。允许客户端通过服务器代理中转所有的 `.ts` / `.mp4` 真实媒体切片（部分强防盗链源如 Anime1 必须开启此项方可播放）。**公网小带宽 VPS 请慎重开启**。
...
### 场景 B：公网云服务器 (VPS) 生产部署（安全加固 + 域名绑定）

防盗刷出站流量，绑定独立域名，日志规范化：
...
# 安全加固：强制设置代理密码，禁止 ts 切片隧道消耗服务器流量
PUBLIC_PROXY=1
PROXY_TOKEN=YourVeryStrongPassword2026!
MEDIA_FULL_PROXY=0
...
# 密码保护
PROXY_TOKEN=YourSecretPasswd!

# 直连海外官方线路
BANGUMI_API=official
BANGUMI_IMAGE=official

# 开启 Anime1 等特殊源的代理中转（若 VPS 流量充足）
MEDIA_FULL_PROXY=1
