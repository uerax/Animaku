...
> 本文档详细记录了在 **Animaku / kazumi-web** 中分析、逆向并接入全新视频源（Video Source / Plugin）的标准步骤、架构模式与踩坑注意事项，供后续维护与开发参考。

---

## 1. 视频源接入架构决策矩阵

在接入新视频源前，首先通过抓包或探查确定其**技术形态**，选择最合适的接入路径：
...
```
   ...
   - 代表：mxdm                    │                                - 代表：tvtfun, cycani, xifan-next
         │                                 │                                │
         ▼                                 ▼                                ▼
...
针对现代 SPA / BaaS / 加密 API 类型的优质视频源（如 `tvtfun`、`cycani`、`xifan-next`），标准接入流程分为以下 6 个步骤：
...
#### 🛡️ 容灾与自愈设计（必加项）
若接口使用了前端静态 Key / 临时 Token，必须增加 **401/403 自动嗅探刷新机制**：
```typescript
...
- 海外 CDN 节点对 HEAD 请求响应缓慢（耗费 1000~2500ms），严重拖慢起播；浏览器原生支持 0ms 跟随 302 重定向。
...
1. **新建 `apps/web/src/data/default-plugins/{name}.json`**：
   ...
   ```
   > **⚠️ 格式规范**：若由专有适配器全权接管解析，JSON 中的 `searchMode` 保持默认 `"xpath"` 并保留合法的 `searchURL`。  
...
| :--- | :--- | :--- |
| **BaaS / Serverless 区域路由** | 未指定机房 region，请求走全球 Edge Ingress 跨洲中继，延迟增加 7 倍（1.7s vs 0.25s）。 | 抓包官方前端查看配置 region（如 `ap-southeast-1`），在 URL 追加 `?forceFunctionRegion=...` 并在 Header 注入 `x-region`。 |
| **流分支串行试错** | 先 `await` HLS（失败报错），再 `await` Fallback，两次串行导致解析耗时成倍放大。 | 改用 `Promise.allSettled` 并发请求 HLS 与 Fallback，取最快可用分支，将耗时压缩至单次请求最大值（~250ms）。 |
