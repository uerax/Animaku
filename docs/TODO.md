...
- [ ] **痛点分析**：当前 `WatchHudToast` 使用全网页顶部固定定位（`fixed top-16 left-1/2`），在宽屏/大屏桌面端与播放器画面严重脱节，且在网页全屏或页面滚动时位置突兀。
- [ ] **对标主流流媒体实践（Bilibili / YouTube / Netflix）**：
...
- [ ] **痛点分析**：当前服务端日志由 Hono 默认 `logger()` 打印，大量被 Docker 每 30s 一次的 `GET /api/health` 刷屏，且缺少真实请求 IP、准确请求时间戳与结构化响应耗时。
- [ ] **优化目标**：
